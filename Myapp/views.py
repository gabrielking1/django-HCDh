...
def login(request):
    ...
    if request.method == 'POST':
        ...
        profile = Profiles.objects.filter(username=user)
        if profile:
            messages.info(request, 'You registered as a socialist not a developer')
...
def profile(request, username):
    if request.user.is_superuser:
        ...
        if apper:
            ...
            followin = Profile.objects.filter(follower=userr)
        ...
        #       {{content|safe }}
            ...
            return render(request,"profile.html",
                        ...
                        'profile':profiler,'notify':notify,'following':following
                        }
                        )
    ...
    #       {{content|safe }}
            if form.is_valid():
                ...
                if Blog.objects.filter(title=title):
                    ...
                    form.save()
                    return redirect('Myapp:question')
...
def read(request):
    ...
    print("this was liked ",notify.id)
    notify.isread = "Read"
    notify.save()
    return redirect(request.META.get('HTTP_REFERER'))
...
#         print("enter ")
#         follows = profile.follower.all()
#         user = request.user
#         print("na profile ne this")
#         if request.method == "POST":
#             leader = request.POST.get('owner')
#             follows = request.POST.get('followers')

            
          
#             if request.user.username== userr.username:
#                 messages.error(request, 'you are cannot follow yourself')
#                 # return redirect(request.META.get('HTTP_REFERER'))
    ...
    if request.user.is_active and request.user.is_authenticated:
        ...
        if request.method == "POST":
            if request.user == userr:
                ...
                if profile.follower.filter(username=request.user).exists():
                    profile.follower.remove(request.user)
                    messages.info(request, 'You have unfollowed {}.'.format(profile.username))
                else:
